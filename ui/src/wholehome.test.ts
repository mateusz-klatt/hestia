...
describe("openWholeHomeConfig", () => {
  ...
  function open(over: Partial<WholeHomeConfigDeps> = {}): () => void {
    return openWholeHomeConfig({ devices, excluded, setExcluded: vi.fn(), ...over });
  }
  ...
  it("unchecking a device opts it OUT (exclude = true)", async () => {
  ...
  it("shows an empty placeholder when there are no lights or blinds", () => {
    openWholeHomeConfig({ devices: { "1": device({ type: "thermostat" }) }, excluded, setExcluded: vi.fn() });
